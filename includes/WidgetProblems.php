...
namespace Modules\ProblemsPages\Includes;
...
class WidgetProblems extends CTableInfo {
	...
	private function build(): void {
		...
		if ($this->data['info'] !== '' || $this->data['page']['cursor_mode']
				|| $this->data['page']['total'] > 1) {
			$footer = (new CDiv())
				->addClass('problem-pages-footer')
				->addStyle('display: block; width: 100%;');

			if ($this->data['info'] !== '') {
				$footer->addItem(
					(new CDiv(
						(new CSpan($this->data['info']))
						->addClass('table-info')
					))
						->addClass('problem-pages-info')
						->addStyle('display: block; width: 100%; text-align: right; margin-bottom: 8px;')
				);
			}

			if ($this->data['page']['cursor_mode'] || $this->data['page']['total'] > 1) {
				$pager = (new CDiv())
					->addClass('problem-pages-nav')
					->addStyle('display: inline-flex; justify-content: flex-end; align-items: center; gap: 6px; flex-wrap: wrap; position: relative; z-index: 20; pointer-events: auto;');

				if ($this->data['page']['cursor_mode']) {
					$pager->addItem($this->makePageControl(_('Previous'), $this->data['page']['has_previous'],
						false, null, 'previous'
					));
					$pager->addItem($this->makePageControl(_s('Page %1$d', $this->data['page']['current']),
						false, true
					));
					$pager->addItem($this->makePageControl(_('Next'), $this->data['page']['has_next'],
						false, null, 'next'
					));
				}
				else {
					$pager->addItem($this->makePageControl(_('Previous'), $this->data['page']['has_previous'],
						false, $this->data['page']['current'] - 1
					));

					foreach ($this->getPageList($this->data['page']['current'], $this->data['page']['total']) as $page) {
						if ($page === null) {
							$pager->addItem((new CSpan('...'))->addClass('problem-pages-gap'));
							continue;
						}

						if ($page === $this->data['page']['current']) {
							$pager->addItem($this->makePageControl((string) $page, false, true));
							continue;
						}

						$pager->addItem($this->makePageControl((string) $page, true, false, $page));
					}
				...
				$footer->addItem(
					(new CDiv($pager))
						->addClass('problem-pages-nav-wrap')
						->addStyle('display: block; width: 100%; text-align: right; clear: both;')
				);
			}
	...
	private function makePageControl(string $label, bool $enabled, bool $is_current = false,
			?int $page = null, ?string $page_action = null) {
		$button = (new CButton(null, $label))
			->addClass('btn-alt')
			->addClass('problem-pages-button')
			->setAttribute('type', 'button')
			->addStyle('position: relative; z-index: 30; pointer-events: auto; min-width: 32px; cursor: pointer;');

		if ($enabled) {
			$button->setAttribute('data-action', 'page');
			...
			if ($page_action !== null) {
				$button->setAttribute('data-page-action', $page_action);
			}
		}
		else {
		...
		if ($is_current) {
			$button->addClass('problem-pages-current');
		}

		return $button;
	}

	private function getPageList(int $current_page, int $total_pages): array {
		if ($total_pages <= 7) {
